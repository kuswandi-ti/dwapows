...
<div id="tab-container">
	<ul id="tab">
		<li><a href="<?= base_url('plo/plo_releases'); ?>" class="active">Open</a></li>
		<li><a href="<?= base_url('plo/plo_histories'); ?>">Received</a></li>
	</ul>
</div>
...
<div id="content-container">
	<ul id="accordion">
		...
		<ul>
			<li>
				<?= form_open('plo/plo_releases/search'); ?>
					<table class="list-table">
						<tr>
							...
							<td>
								...
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								<a href="<?= base_url('plo/plo_releases'); ?>" class="css-button">Refresh & View All</a>
							</td>
						</tr>
					</table>
				<?= form_close(); ?>
        ...
        <table border="0" class="list-table custom-table">
			...
			?>
				...
				<?php
					...
					if ($plo_status!=$this->config->item('status_released_info')) { ?>
				...
				<td align='center'>
					<?php
						if ($plo_status==$this->config->item('status_rejected_info').$sess_userid) { ?>
							No Action
						<?php } else { ?>
	...
	<!--<embed 
		src="<?= $this->config->item('url_upload').$row['pdf_file']; ?>"
		quality="high"
		name="fb"
	...
	<div id="form-receive" title="Receive PLO">Are you sure to receive data ?</div>
	<div id="form-reject" title="Reject PLO">
		...
		$( "#form-receive" ).dialog({						
			...
			buttons: {				
				"Yes": function() {	
					...
					$.ajax({
						type: 'POST',
						url: "<?= base_url('plo/plo_releases/receive'); ?>",	
						data: form_data,
						dataType: "json",
						async: false,
						success: function(msg) {
							$('#id').val('');
							$(location).attr('href',"plo/plo_releases/download/"+msg.res);	
							alert('Successfully receive PLO Document !!!. Please download & Print PDF document.');
							$("#form-receive").dialog("close");
							setTimeout(function() {
								$(location).attr('href',"<?= base_url('plo/plo_releases'); ?>");
							}, 5000);							
						}
					});
					return false;
		...
		$( "#form-reject" ).dialog({						
			...
			buttons: {				
				"Yes": function() {	
					...
					$.ajax({						
						type: 'POST',
						url: "<?= base_url('plo/plo_releases/reject'); ?>",	
						data : form_data,
						async: false,
						success: function(msg) {
							$('#id').val('');
